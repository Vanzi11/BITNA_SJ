...
> **🤖 Para IAs continuando este trabalho: leia `GOVERNANCA_DA_SQUAD_BITNA.md` e `CONTINUIDADE.md` PRIMEIRO.** Eles contêm a governança operacional da Squad Bitna (Naia), o estado do projeto, as regras invioláveis da marca, a tarefa pendente prioritária e a ordem de leitura dos documentos. Cada pasta tem seu próprio README.

Empresa digital de relatórios de autoconhecimento baseados no Saju (사주, Quatro Pilares do Destino) e sinastria (궁합, gunghap), adaptados para o público brasileiro. Operada pela **Squad Bitna (Naia)** sob a governança de `GOVERNANCA_DA_SQUAD_BITNA.md` (ver também `NAIA.md`). Portfólio, preços reais em produção (Hotmart, conferidos em `empresa/LINKS_HOTMART.md`): Leitura Essencial R$ 47,60 · Leitura Completa R$ 149,30 · Sinastria Amorosa R$ 98,00 · Sinastria Profissional R$ 98,00 · Jornadas Bitna R$ 249 em produção atual, com novo preço aprovado em D64 de R$ 332,00 pendente de refletir no cadastro/oferta/checkout · Guia Ilustrado do Saju R$ 9,80 (decidido, em construção). Ver `empresa/EMPRESA.md` e a auditoria de coerência em `empresa/AUDITORIA_COERENCIA_PRODUTOS.md`.
...
| `app/` | Backend HTTP do produto + UI de teste (`node --env-file-if-exists=app/.env app/server.mjs` → http://localhost:3333) |
