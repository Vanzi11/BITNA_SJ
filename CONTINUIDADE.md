...
- ~~Publicação no cPanel e o próprio backend da Fase 5 (webhook, formulário público real, storage de pedido, e-mail, hospedagem Railway — ver D44) continuam como próximos passos~~ — ✅ **concluído (15/08/2026)**: as 8 páginas estão publicadas em `bitnasaju.com.br` e o backend roda em produção no Railway (D52–D55). Ver a seção "PUBLICADO EM PRODUÇÃO" acima — esta linha só ficou desatualizada (corrigida em 13/09/2026).

## Demais pendências (ordem sugerida)

0. **[D58] Construir o Guia Ilustrado do Saju** (R$9,80) — produto novo decidido (não hipótese), candidato a order bump da Essencial/Completa. Falta tudo: prompt, motor/conteúdo e gerador de PDF. Ver `empresa/EMPRESA.md` e `empresa/CONTEXTO_E_AJUSTES_PROJETO_COMERCIAL_BITNA_SAJU_V1.md` (seção 4).
